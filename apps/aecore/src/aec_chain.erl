...
-define(IS_WORK(W), (is_integer(W) andalso (W > 0))). %% For guard.
...
do_init_check_genesis_block(GenesisBlock) ->
    ...
    Difficulty = aec_blocks:difficulty(GenesisBlock),
            ...
            {error, not_found} ->
                ...
                {ok, HeadersDb2} =
                    headers_db_put(
                      ...
                      #chain_header{
                         h = GenesisHeader,
                         td = aec_headers:difficulty(GenesisHeader)}),
                {ok, BlocksDb2} =
...
do_insert_header(Header, TopHeader, TopHeaderDb, HeadersDb) ->
    HeaderDifficulty = aec_headers:difficulty(Header),
